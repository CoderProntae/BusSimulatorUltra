...
## How far ahead the car looks, and the bubble it refuses to enter.
const DETECT_DISTANCE: float = 14.0
const STOP_DISTANCE: float = 5.5

var _detector: RayCast3D = null
var _blocked: bool = false
## 0 = something right in front, 1 = clear road. Scales the target speed.
var _gap_ratio: float = 1.0
...
func _physics_process(delta: float) -> void:
	...
	# Proportional following instead of the old all-or-nothing stop.
	# _update_blocked() used to be a single boolean from one short ray, so a
	# car either drove at full speed or slammed to zero, then sat there
	# jittering behind the bus. Now the gap scales the speed.
	_update_blocked()
	if _gap_ratio < 1.0:
		target_speed = minf(target_speed, speed * _gap_ratio)
	if _blocked:
		target_speed = 0.0

	# Brake much harder than we accelerate, like a real driver.
	var accel: float = 5.5
...
func _update_blocked() -> void:
	## Works out how much room there is ahead and turns it into _gap_ratio
	## (0 = stop now, 1 = road is clear), so the car eases off instead of
	## flip-flopping between full speed and a dead stop.
	_blocked = false
	_gap_ratio = 1.0
	...
	if not (node.is_in_group("bus") or node.is_in_group("traffic")):
		return

	var point: Vector3 = _detector.get_collision_point()
	var distance: float = global_position.distance_to(point)

	# Hard stop inside the safety bubble, otherwise scale down over the
	# remaining detector length.
	if distance <= STOP_DISTANCE:
		_blocked = true
		_gap_ratio = 0.0
		return
	_gap_ratio = clampf(
		(distance - STOP_DISTANCE) / (DETECT_DISTANCE - STOP_DISTANCE), 0.0, 1.0)
...
func _build_detector() -> void:
	...
	# Start the ray at the front bumper, not the car's centre, otherwise the
	# first 2 m of "gap" is just the car's own bodywork.
	ray.position = Vector3(0.0, 0.75, -2.1)
	ray.target_position = Vector3(0.0, 0.0, -DETECT_DISTANCE)
	ray.enabled = true
	ray.collide_with_areas = false
	ray.collide_with_bodies = true
	# Do not let the ray hit our own collision shape.
...
func _detect_collisions() -> void:
	...
	while i < count:
		...
		# A bus is ~10 t against a ~1.2 t car, so weight the hit by how much
		# heavier the thing that hit us is. Without this a bus at 40 km/h
		# ramming a car doing 38 km/h in the same direction registered almost
		# no impact at all and the car just shrugged it off.
		var mass_factor: float = 1.0
		if node.is_in_group("bus"):
			mass_factor = 4.5
		_register_impact(impact * mass_factor, col.get_normal())
		i += 1


func _register_impact(impact: float, normal: Vector3) -> void:
	## Shared by our own move_and_slide() contacts and by hits reported TO us
	## by the bus (see take_external_hit).
	if impact > 5.0:
		_take_damage(impact * 9.0, normal)
	elif impact > 1.0:
		_begin_reverse()


func take_external_hit(closing_speed: float, normal: Vector3) -> void:
	## Called by the bus when IT detects the collision.
	##
	## THIS IS THE FIX for "I hit a car and nothing happened". A
	## CharacterBody3D only reports contacts generated by its OWN
	## move_and_slide(). When the bus drove into a car the car was often
	## stationary or already braking, so its own move_and_slide() produced no
	## contact and _detect_collisions() never ran. The damage therefore
	## depended on the victim happening to be driving into the bus, which is
	## exactly backwards. Now the bus, which is the one with the momentum,
	## reports the hit.
	if car_state == CarState.EXPLODED:
		return
	_register_impact(absf(closing_speed) * 4.5, normal)
